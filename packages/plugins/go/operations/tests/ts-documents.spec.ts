...
import { validateGo } from '../../go/tests/validate';
import { plugin as tsPlugin } from '../../go/src';
import { compileGo } from '../../go/tests/compile';
import { mergeOutputs, Types } from '@graphql-codegen/plugin-helpers';

describe('Go Operations Plugin', () => {
  ...
  const validate = async (content: Types.PluginOutput, config: any = {}, pluginSchema = schema) => validateGo(mergeOutputs([await tsPlugin(pluginSchema, [], config, { outputFile: '' }), content]));
  ...
  `);
    ...
    it('Should generate the correct intersection for fragments when using with interfaces with same type', async () => {
      ...
      compileGo(mergeOutputs([result]), config);
    });
